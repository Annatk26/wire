...
if __name__ == "__main__":
    utils.log(
        "Starting image denoising experiment"
    )
    weight_init = False
    plt.gray()
    ...
    # sigma0 = 9.5522
    sigma0_all = [4.0, 9.0, 12.0]  # sigma0 = 0.5
    scale_tensor_all = [[4.0, 8.0, 12.0, 16.0, 24.0, 32.0], [2.0, 8.0, 14.0, 20.0, 28.0], [6.0, 15.0, 24.0, 33.0, 45.0]]
    learning_rate_all = [2e-2, 4e-3]   # Learning rate
    ...
    for scale_tensor in scale_tensor_all:
        for sigma0 in sigma0_all:
            for learning_rate in learning_rate_all:
                scaled_hidden_features = int(hidden_features / len(scale_tensor))
                maxpoints = int(np.mean([scaled_hidden_features, hidden_features]))   # Batch size
                    
                utils.log(f"System Information")
                utils.log(
                    f"Non-linearity: {nonlin}, Learning Rate: {learning_rate}, Scale: {sigma0}"
                )
                utils.log(f"Scale tensor: {scale_tensor}, Hidden features (scaled layer): {scaled_hidden_features}")

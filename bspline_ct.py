...
if __name__ == "__main__":
    ...
    # Network parameters
    hidden_layers = 2
    hidden_features = curr_config[
        "hidden_features"
    ]  # TODO: Number of hidden units per layer (300)
    ...
    img = cv2.resize(img, None, fx=scale_im, fy=scale_im, interpolation=cv2.INTER_AREA)
    [H, W] = img.shape
    ...
    if isinstance(learning_rate, list):
        ...
        for i, stage in enumerate(model.stages):
            param_groups.append({"params": stage.parameters(), "lr": learning_rate[i]})
            param_groups.append(
                {"params": model.linears[i].parameters(), "lr": learning_rate[i]}
            )
    ...
    folder_name = utils.make_unique(
        f"{curr_config['name']}",
        "/rds/general/user/atk23/home/wire/multiscale_results/ct",
    )

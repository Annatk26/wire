...
def get_IoU_batch(preds, gt, thres=None, maxpoints=pow(2, 24)):
    ...
    for b_idx in range(0, preds.numel(), maxpoints):
        ...
        intersection, union = get_I_and_U(preds[b_idx:b_idx2], gt[b_idx:b_idx2], thres)
...
def export_mesh(
    coords, cube_res, model, batchsize, savename, occupancy=None, thres=0.005
):
    """
...
def get_query_coords(minlim, maxlim, cube_res, depth):
    ...
    X, Y, Z = np.meshgrid(x, y, z)
    coords_gen = np.hstack((X.reshape(-1, 1), Y.reshape(-1, 1), Z.reshape(-1, 1)))
...
class BlockPCDDataset(object):
    ...
    def __init__(self, xyz, sdf=None, depth=4):
        ...
        self.chunks, self.sdf, self.minlim, self.maxlim, self.indices = self.build()
    ...
    def build(self, xyz=None, depth=None):
        ...
        all_idx = np.hstack((X.reshape(-1, 1), Y.reshape(-1, 1), Z.reshape(-1, 1)))
    ...
    def fill(self, chunks=None, sdf=None):
        ...
        for key in chunks.keys():
            ...
            newpts = [
                minlim + (maxlim - minlim) * np.random.rand(1, 3) for _ in range(addpts)
            ]
    ...
    def unfold(self, sdfs, sdfcube=None, cubesize=None):
        ...
        for key in self.indices:
            ...
            block = sdfs[self.indices[key], :, 0].reshape(
    ...
    def downsample(self, chunks=None, ndepths=1, targetdepth=None):
        ...
        for idx in range(ndepths):
            ...
            # Average
            chunks_ds = {key: [np.vstack(item).mean(0)] for key, item in chunks.items()}
            # Collate
            chunks_flat = self.flatten(chunks_ds)

            # Subdivide
            chunks, sdf, minlim, maxlim, indices = self.build(
                chunks_flat, self.depth - idx - 1
            )

        # If target depth is other than None, redivide
        if targetdepth is not None:
            chunks, sdf, minlim, maxlim, indices = self.build(chunks_flat, targetdepth)

        return chunks, sdf, minlim, maxlim, indices


def get_occupancy_cube(
    cube_res, sidelength, pred_occupancy, model_input, display_occupancy=None
):
    # get voxel idx for each coordinate
    coords = model_input["fine_abs_coords"]
    if type(coords) == torch.Tensor:
        coords = coords.cpu().numpy()
    voxel_idx = np.floor((coords + 1.0) / 2.0 * (sidelength)).astype(np.int32)
    voxel_idx = voxel_idx.reshape(-1, 3)

    # init a new occupancy volume
    if display_occupancy is None:
        display_occupancy = -1 * np.ones(
            (cube_res, cube_res, cube_res), dtype=np.float32
        )
    else:
        display_occupancy[...] = -1

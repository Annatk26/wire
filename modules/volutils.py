...
def add_noise(volume, max_shift):
    ...
    coords = np.hstack((Xn.reshape(-1, 1), Yn.reshape(-1,
                                                      1), Zn.reshape(-1, 1)))
...
def get_query_coords(minlim, maxlim, cube_res, depth):
    ...
    X, Y, Z = np.meshgrid(x, y, z)
    coords_gen = np.hstack((X.reshape(-1, 1), Y.reshape(-1,
                                                        1), Z.reshape(-1, 1)))
...
class BlockPCDDataset(object):
    ...
    def build(self, xyz=None, depth=None):
        ...
        all_idx = np.hstack((X.reshape(-1, 1), Y.reshape(-1,
                                                         1), Z.reshape(-1, 1)))
    ...
    def refill(self, folded_chunks, folded_sdfs, minlim, maxlim):
        ...
        folded_chunks = folded_sdfs * folded_chunks + (1 -
                                                       folded_sdfs) * newpts
    ...
    def unfold(self, sdfs, sdfcube=None, cubesize=None):
        ...
        if sdfcube is None:
            sdfcube = np.zeros((cubesize, cubesize, cubesize),
                               dtype=np.float32)
        else:
        ...
        for key in self.indices:
            ...
            block = sdfs[self.indices[key], :,
                         0].reshape(blocksize, blocksize, blocksize)
    ...
    def downsample(self, chunks=None, ndepths=1, targetdepth=None):
        ...
        if targetdepth is not None:
            chunks, sdf, minlim, maxlim, indices = self.build(
                chunks_flat, targetdepth)

        return chunks, sdf, minlim, maxlim, indices


def get_occupancy_cube(cube_res,
                       sidelength,
                       pred_occupancy,
                       model_input,

...
class ImageSRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.Xstack is not None:
            coords = torch.stack((torch.tensor(
                self.Xstack[idx, ...]), torch.tensor(self.Ystack[idx, ...])),
                                 dim=-1).reshape(-1, 2)
        else:
...
def ecc_flow(im1, im2, warp_mode=cv2.MOTION_HOMOGRAPHY, niters=1000, eps=-1):
    ...
    (cc, warp_matrix) = cv2.findTransformECC(im1,
                                             im2,
                                             warp_matrix,
                                             warp_mode,
                                             criteria=criteria,
...
def get_SR_data(im,
    ...
    if simulation is True:
        H, W = im.shape
        imstack, _, _, mats = get_imstack(im, scale, shift_max, theta_max,
                                          nimg)
...
def get_imstack(im, scale, shift_max=10, theta_max=np.pi / 12, nshifts=5):
    ...
    coords = np.hstack((X.reshape(-1, 1), Y.reshape(-1, 1), np.ones(
        (H * W, 1))))
...
def get_downsampled_shifted_images(im,
                                   scale,
                                   shift_max=10,
                                   theta_max=np.pi / 12,
                                   nshifts=5,
...
def register_stack(imstack, full_res, method=StackReg.RIGID_BODY):
    ...
    reg = StackReg(method)
    reg_mats = reg.register_stack(imstack_full,
                                  reference='first',
                                  verbose=True)
...
def mat2coords(reg_stack, full_res, low_res):
    ...
    for idx in range(nimg):
        ...
        coords_new = mat.dot(coords.T).T
        Xstack[idx,
               ...] = cv2.resize(2 * coords_new[:, 0].reshape(H, W) / W - 1,
                                 (Wl, Hl),
                                 interpolation=cv2.INTER_AREA)
        Ystack[idx,
               ...] = cv2.resize(2 * coords_new[:, 1].reshape(H, W) / H - 1,
                                 (Wl, Hl),
                                 interpolation=cv2.INTER_AREA)
...
def affine2rigid(mats):
    ...
    if False:
        angles = (np.arccos(mats[:, 0, 0]) - np.arcsin(mats[:, 0, 1]) +
                  np.arcsin(mats[:, 1, 0]) + np.arccos(mats[:, 1, 1])) / 4.0
...
def register_stack_ecc(imstack, full_res, method=cv2.MOTION_EUCLIDEAN):
    ...
    for idx in tqdm.tqdm(range(1, nimg)):
        ...
        try:
            ...
            alignment_err[idx] = abs(
                (imref - im2_aligned) * spatial_mask).mean()
        except:
            mask[idx] = 0
            continue

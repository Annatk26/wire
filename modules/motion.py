...
def getEuclidianMatrix(theta, shift):
    ...
    mat = np.array(
        [
            [np.cos(theta), -np.sin(theta), shift[0]],
...
def ecc_flow(im1, im2, warp_mode=cv2.MOTION_HOMOGRAPHY, niters=1000, eps=-1):
    ...
    if warp_mode == cv2.MOTION_HOMOGRAPHY:
        ...
        im2_aligned = cv2.warpPerspective(im2, warp_matrix, (sz[1], sz[0]), flags=flags)
    else:
        # Use warpAffine for Translation, Euclidean and Affine
        flags = cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP
        im2_aligned = cv2.warpAffine(im2, warp_matrix, (sz[1], sz[0]), flags=flags)
...
def get_SR_data(
    ...
    if simulation is True:
        H, W = im.shape
        imstack, _, _, mats = get_imstack(im, scale, shift_max, theta_max, nimg)
        ...
        if downsample:
            ...
            for idx in range(nimg):
                imstack_lr[idx, ...] = cv2.resize(
                    imstack[idx, ...], (W // scale, H // scale), interpolation=interp
                )
            im = imstack[0, ...]
...
def get_imstack(im, scale, shift_max=10, theta_max=np.pi / 12, nshifts=5):
    ...
    for idx in range(nshifts):
        ...
        imstack[idx, ...] = cv2.remap(
            im, Xnew.astype(np.float32), Ynew.astype(np.float32), cv2.INTER_LINEAR
        )
...
def get_downsampled_shifted_images(
    im, scale, shift_max=10, theta_max=np.pi / 12, nshifts=5, perturb_coords=False
):
    """
    Obtain synthetically generated, low resolution images of im, with
    random shifts.
    ...
    for idx in range(nshifts):
        ...
        # Rotate
        Xrot = (
            (Xshifted - W / 2) * np.cos(theta)
            - (Yshifted - H / 2) * np.sin(theta)
            + W / 2
        )
        Yrot = (
            (Xshifted - W / 2) * np.sin(theta)
            + (Yshifted - H / 2) * np.cos(theta)
            + H / 2
        )
        ...
        imstack[idx, ...] = cv2.remap(
            im, Xnew.astype(np.float32), Ynew.astype(np.float32), cv2.INTER_AREA
        )
...
def register_stack(imstack, full_res, method=StackReg.RIGID_BODY):
    ...
    # Upsample the images
    for idx in range(nimg):
        imstack_full[idx, ...] = cv2.resize(
            imstack[idx, ...], (Wr, Hr), interpolation=cv2.INTER_AREA
        )
...
def mat2coords(reg_stack, full_res, low_res):
    ...
    for idx in range(nimg):
        ...
        coords_new = mat.dot(coords.T).T
        Xstack[idx, ...] = cv2.resize(
            2 * coords_new[:, 0].reshape(H, W) / W - 1,
            (Wl, Hl),
            interpolation=cv2.INTER_AREA,
        )
        Ystack[idx, ...] = cv2.resize(
            2 * coords_new[:, 1].reshape(H, W) / H - 1,
            (Wl, Hl),
            interpolation=cv2.INTER_AREA,
        )
...
def interp_lr(imref, coords, renderer):
    ...
    B = coords.shape[0]
    im_hr = F.grid_sample(
        torch.repeat_interleave(imref, B, 0),
        coords,
        mode="bilinear",
        align_corners=False,
    )
...
def register_stack_ecc(imstack, full_res, method=cv2.MOTION_EUCLIDEAN):
    ...
    imref = cv2.resize(imstack[0, ...], (Wr, Hr), interpolation=cv2.INTER_LINEAR)
    mask[0] = 1
    ...
    for idx in tqdm.tqdm(range(1, nimg)):
        im2 = cv2.resize(imstack[idx, ...], (Wr, Hr), interpolation=cv2.INTER_LINEAR)
        try:
            mat, im2_aligned, flow = ecc_flow(imref, im2, warp_mode=method)
            mask[idx] = 1
            ecc_mats[idx, :] = mat
            Xstack[idx, ...] = cv2.resize(
                X - flow[..., 0], (W, H), interpolation=cv2.INTER_LINEAR
            )

            Ystack[idx, ...] = cv2.resize(
                Y - flow[..., 1], (W, H), interpolation=cv2.INTER_LINEAR
            )

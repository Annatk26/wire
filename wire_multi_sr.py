...
if __name__ == '__main__':
    nonlin_types = ['wire', 'siren', 'mfn', 'relu', 'posenc', 'gauss']            # type of nonlinearity, 'wire', 'siren', 'mfn', 'relu', 'posenc', 'gauss'
    niters = 2000               # Number of SGD iterations (2000)
    # learning_rate = 5e-3        # Learning rate. 

    os.makedirs('/rds/general/user/atk23/home/wire/results/multi_SR', exist_ok=True)
    ...
    # Read image
    im = cv2.resize(plt.imread('/rds/general/user/atk23/home/wire/data/kodak.png'), None,
    ...
    for nonlin in nonlin_types:
        utils.log(f'Starting muilti-SR with {nonlin}')
        learning_rate = {
            'wire': 5e-3, 
            'siren': 1e-3, 
            'mfn': 5e-2, 
        ...
        for idx in range(3):
            utils.log(f'Points: {points.shape} | Image: {imstack[:, idx, ...].reshape(-1, 1).shape}')
        ...
        filename = f'/rds/general/user/atk23/home/wire/results/multi_SR/{scale_sr}x_{nimg}images_{nonlin}'
        ...
        mdict[nonlin] = {'rec': img_full,
                'psnr_rec': snrval,
                'ssim_rec': ssimval,
                'psnr_interp': snr_interp,
                'ssim_interp': ssim_interp,
                'rec_interp': im_interp}
        io.savemat(f'/rds/general/user/atk23/home/wire/results/multi_SR/{filename}.mat', mdict[nonlin])
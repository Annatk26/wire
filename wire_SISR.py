...
if __name__ == "__main__":
    ...
    folder_name = utils.make_unique(
        "sisr", "/rds/general/user/atk23/home/wire/baseline_results"
    )
    os.makedirs(
    ...
    # Read image
    im = utils.normalize(
        plt.imread("/rds/general/user/atk23/home/wire/data/butterfly.png").astype(
            np.float32
        ),
        True,
    )
    ...
    for i, nonlin in enumerate(nonlin_types):
        ...
        metrics[nonlin] = {
            ...
            "MSE Difference": abs(
                (-10 * torch.log10(best_mse).item()) - expected["Expected MSE"][i]
            ),
            "SSIM Difference": abs(
                ssim_func(im, best_img, multichannel=True)
                - expected["Expected SSIM"][i]
            ),
